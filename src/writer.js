...
async function loadMarkdownFilePromise(post) {
	...
	*/
//Return all Arrays with Proper formatting ["item 1", "item 2", "item 3", "etc"]
//This takes care of both Categories and Tags Frontmatter
async function loadMarkdownFilePromise(post) {
  let output = "---\n";
...
function getPostPath(post, config) {
  ...
  if (settings.custom_date_formatting) {
    dt = luxon.DateTime.fromFormat(
      post.frontmatter.date,
      settings.custom_date_formatting,
    );
  } else {
